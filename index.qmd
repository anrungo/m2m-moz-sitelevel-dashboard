...
// upload data
comp1_final = FileAttachment("data/final/merged_all_data.csv").csv({ typed: true })
...
enrichedData = filteredMonth.map((d) => {
  ...
  const columnsToConvert = Object.keys(d).filter((key) => 
    key.startsWith("%") || 
...
data_tbl = aq.from(data)

data_enriched_tbl = aq.from(filteredMonth)


data_longer_tbl = data_enriched_tbl
  .select(
    "Provincia",
    "Unidade Sanitaria",
    "Periodo",
...
data_eid_longer_tbl = data_enriched_tbl
  .select(
    "Provincia",
    "Unidade Sanitaria",
    "Periodo",
...
data_suppression_tbl = data_enriched_tbl
  .select(
    "Provincia",
    "Unidade Sanitaria",
    "Periodo",
...
data_cad_tbl = data_enriched_tbl
  .select(
    "Provincia",
    "Unidade Sanitaria",
    "Periodo",
...
cad_ret_data = data_cad_tbl.filter((d) => d.category.includes("Levantamento"))
cad_teste_data = data_cad_tbl.filter((d) => d.category.includes("teste"))
cad_result_data = data_cad_tbl.filter((d) => d.category.includes("resultados"))
...
Este painel apresenta o desempenho do programa a nível da unidade sanitária:
...
viewof pickProvince = Inputs.radio(
  [...new Set(comp1_final.map((d) => d.Provincia))],
...
//| output: false
selectedProv = comp1_final.filter((d) => d.Provincia === pickProvince)
```
...
viewof selectedMonth = Inputs.select(
  Array.from(new Set(selectedHF.map((d) => d.Periodo))),
  {
    label: "Filtrar por mês",
    multiple: true
    // value: [selectedHF[9].Periodo]
...
//| output: false
filteredMonth = selectedHF.filter((d) => {
  const monthMatch =
...
::: {.card title="Cascata de Testagem Precoce Infantil"}
```{ojs}
Plot.plot({
 height: 420,
  style: {
...
::: {.card title="Ligação e Suppressão Viral"}
```{ojs}
Plot.plot({
 height: 420,
  x: { axis: null },
  ...
  marks: [
    Plot.barY(data_suppression_tbl, {
      x: "Periodo",
      y: "percent",
      fill: "Periodo",
      fx: "category",
    ...
    Plot.text(data_suppression_tbl, {
      x: (d) => d.Periodo,
      y: "percent",
...
::: {.card title="Levantamento de ARV"}
```{ojs}
Plot.plot({
 height: 420,
  x: { axis: null },
...
::: {.card title="Testagem de Carga Viral"}
```{ojs}
Plot.plot({
 height: 420,
  x: { axis: null },
...
::: {.card title="Resultado de Carga Viral"}
```{ojs}
Plot.plot({
 height: 420,
  x: { axis: null },
